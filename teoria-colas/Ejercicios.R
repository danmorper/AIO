...
i_mm1 <- NewInput.MM1(lambda=8, mu=12)#Se puede atender de una media en 5 minutos  
i_mm5 <- NewInput.MM1(lambda=8, mu=12, n=5) #Bastaria con n=1 pero ponemos 5 para iltustrar como funciona

# Optionally check the inputs of the model
CheckInput(i_mm1) #si cambias a lambda=4 y mu=3 te da fatal error
# Create the model
o_mm1 <- QueueingModel(i_mm1)  
o_mm5 <- QueueingModel(i_mm5)  #Te devuelve los psubi en $Pn
